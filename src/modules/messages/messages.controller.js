...
export async function createDirectConversation(req,res,next){
  try{
    const conv = await svc.getOrCreateDirectConversation(req.user.id, req.body.targetUserId);
...
export async function getMessages(req,res,next){
  try{
    ...
    const result = await svc.getMessages(req.params.id, req.user.id, {page,limit});
    return paginated(res,{data:result.messages, page, limit, total:result.total});
  }catch(e){next(e);}
}
